...
## Streamlining SDD with Technocrat CLI

The SDD methodology is significantly enhanced through Technocrat's CLI commands that automate the specification → planning → tasking workflow:

### The `technocrat create-feature` Command
...
### The `technocrat setup-plan` Command

Once a feature specification exists, this command creates a comprehensive implementation plan:

1. **Specification Analysis**: Reads and understands the feature requirements from `spec.md`
2. **Constitutional Compliance**: Ensures alignment with project constitution and architectural principles
3. **Technical Translation**: Copies the plan template for you to fill in technical architecture and implementation details
4. **Detailed Documentation**: Provides structure for data models, API contracts, and test scenarios
5. **Quickstart Validation**: Template includes sections for key validation scenarios
...
### The `technocrat update-agent-context` Command

After a plan is created, this command updates your AI agent's context files with the current feature information:

1. **Inputs**: Reads `plan.md` (required) to extract technology stack and architecture decisions
2. **Context Updates**: Updates agent-specific files (CLAUDE.md, .github/copilot-instructions.md, etc.)
3. **Feature Tracking**: Keeps AI agents informed about current feature, tech stack, and recent changes
4. **Output**: Updates existing agent context files with latest project information
...
**SDD with Technocrat Approach:**

```bash
# Step 1: Create the feature specification (5 minutes)
technocrat create-feature "real-time chat system with message history"

# This automatically:
# - Creates branch "003-real-time-chat-system-with-message-history"
# - Generates specs/003-real-time-chat-system-with-message-history/
# - Copies spec.md template for you to fill in

# Step 2: Fill in spec.md with requirements, then generate plan
technocrat setup-plan

# Step 3: Fill in plan.md with tech choices, then update agent context
technocrat update-agent-context

# This automatically updates:
# - Your AI agent's context files
...
```text
Every feature in Technocrat projects SHOULD begin its existence as a standalone library
when appropriate. Features should be implemented with clear boundaries and interfaces
that allow for reusability and testability.
```
